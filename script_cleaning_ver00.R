...
unnecessary_columns = c(
  'identificador_da_ocorrencia', 'identificador_do_individuo', 'ficha_de_campo', 'data_do_obito', 'participantes', 
  'peso_no_momento_da_necropsia', 'peso', 'metodo', 'local_da_necropsia', 'historico', 'observacoes', 'fotos', 
  'motivo_da_nao_coleta_conteudo_gastrointestinal', 'amostras_para_historia_natural_dentes', 'amostras_para_historia_natural_gonadas', 
  'amostras_para_historia_natural_umero_falange', 'destinacao_da_carcaca', 'data', 'local', 'ponto_lat', 'ponto_long', 'fotos_1', 'arquivos', 
  'diagnostico_presuntivo_observaoes', 'diagnostico_primario_observaoes', 'diagnostico_contributivo_observaoes', 'relatorio_administrativo', 
  'mesorregiao', 'habitat', 'local_trecho_id', 'local_trecho_nome', 'local_praia_id', 'local_praia_nome', 'local_cidades_i_ds', 'local_cidades_nomes', 
  'local_estados_i_ds', 'local_estados_nomes', 'local_tipo', 'local_estrategia', 'local_metros', 'pmp'
)

model_columns = c(
  'codigo', 'condicao_da_carcaca', 'necropsia_imediata', 'escore_corporal', 'interacao_tipo', 'interacao_nivel', 'presenca_de_residuos_solidos', 
  ...
  'necropsia_sexo', 'necropsia_estagio_de_desenvolvimento', 'necropsia_classe_do_individuo', 'necropsia_especie_do_individuo'
)
...
df <- extract(df, interacao_remover, into = c('interacao_nivel', 'interacao_remover'), regex = regexNivel, remove = FALSE)
...
df <- extract(df, cavidades_corporeas, into = c('cc_status', 'cc'), regex = regexStatus, remove = FALSE)
df <- extract(df, cc, into = c('cc_remover', 'cc_motivo', 'cc_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('cc', 'cc_remover'))

# Separando tecido_cutaneo_e_subcutaneo em tcs_status, tcs_motivo e tcs_analise
df <- extract(df, tecido_cutaneo_e_subcutaneo, into = c('tcs_status', 'tcs'), regex = regexStatus, remove = FALSE)
df <- extract(df, tcs, into = c('tcs_remover', 'tcs_motivo', 'tcs_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('tcs', 'tcs_remover'))

# Separando sistema_musculo_esqueletico em sme_status, sme_motivo e sme_analise
df <- extract(df, sistema_musculo_esqueletico, into = c('sme_status', 'sme'), regex = regexStatus, remove = FALSE)
df <- extract(df, sme, into = c('sme_remover', 'sme_motivo', 'sme_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('sme', 'sme_remover'))

# Separando sistema_respiratorio em sres_status, sres_motivo e sres_analise
df <- extract(df, sistema_respiratorio, into = c('sres_status', 'sres'), regex = regexStatus, remove = FALSE)
df <- extract(df, sres, into = c('sres_remover', 'sres_motivo', 'sres_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('sres', 'sres_remover'))

# Separando sistema_cardiovascular em sc_status, sc_motivo e sc_analise
df <- extract(df, sistema_cardiovascular, into = c('sc_status', 'sc'), regex = regexStatus, remove = FALSE)
df <- extract(df, sc, into = c('sc_remover', 'sc_motivo', 'sc_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('sc', 'sc_remover'))

# Separando aparato_digestorio em ad_status, ad_motivo e ad_analise
df <- extract(df, aparato_digestorio, into = c('ad_status', 'ad'), regex = regexStatus, remove = FALSE)
df <- extract(df, ad, into = c('ad_remover', 'ad_motivo', 'ad_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('ad', 'ad_remover'))

# Separando sistema_urinario em su_status, su_motivo e su_analise
df <- extract(df, sistema_urinario, into = c('su_status', 'su'), regex = regexStatus, remove = FALSE)
df <- extract(df, su, into = c('su_remover', 'su_motivo', 'su_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('su', 'su_remover'))

# Separando sistema_reprodutivo em srep_status, srep_motivo e srep_analise
df <- extract(df, sistema_reprodutivo, into = c('srep_status', 'srep'), regex = regexStatus, remove = FALSE)
df <- extract(df, srep, into = c('srep_remover', 'srep_motivo', 'srep_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('srep', 'srep_remover'))

# Separando sistema_linfo_hematopoietico em slh_status, slh_motivo e slh_analise
df <- extract(df, sistema_linfo_hematopoietico, into = c('slh_status', 'slh'), regex = regexStatus, remove = FALSE)
df <- extract(df, slh, into = c('slh_remover', 'slh_motivo', 'slh_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('slh', 'slh_remover'))

# Separando sistema_endocrino em se_status, se_motivo e se_analise
df <- extract(df, sistema_endocrino, into = c('se_status', 'se'), regex = regexStatus, remove = FALSE)
df <- extract(df, se, into = c('se_remover', 'se_motivo', 'se_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('se', 'se_remover'))

# Separando orgaos_dos_sentidos em os_status, os_motivo e os_analise
df <- extract(df, orgaos_dos_sentidos, into = c('os_status', 'os'), regex = regexStatus, remove = FALSE)
df <- extract(df, os, into = c('os_remover', 'os_motivo', 'os_analise'), regex = regexMotivo, remove = FALSE)
df <- select(df, -c('os', 'os_remover'))

# Separando sistema_nervoso_central em snc_status, snc_motivo e snc_analise
df <- extract(df, sistema_nervoso_central, into = c('snc_status', 'snc'), regex = regexStatus, remove = FALSE)
df <- extract(df, snc, into = c('snc_remover', 'snc_motivo', 'snc_analise'), regex = regexMotivo, remove = FALSE)
...
df <- df %>% mutate(interacao_tipo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', interacao_tipo))))
df <- df %>% mutate(cc_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', cc_motivo))))
df <- df %>% mutate(cc_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', cc_analise))))
df <- df %>% mutate(tcs_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', tcs_motivo))))
df <- df %>% mutate(tcs_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', tcs_analise))))
df <- df %>% mutate(sme_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sme_motivo))))
df <- df %>% mutate(sme_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sme_analise))))
df <- df %>% mutate(sres_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sres_motivo))))
df <- df %>% mutate(sres_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sres_analise))))
df <- df %>% mutate(sc_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sc_motivo))))
df <- df %>% mutate(sc_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', sc_analise))))
df <- df %>% mutate(ad_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', ad_motivo))))
df <- df %>% mutate(ad_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', ad_analise))))
df <- df %>% mutate(su_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', su_motivo))))
df <- df %>% mutate(su_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', su_analise))))
df <- df %>% mutate(srep_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', srep_motivo))))
df <- df %>% mutate(srep_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', srep_analise))))
df <- df %>% mutate(slh_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', slh_motivo))))
df <- df %>% mutate(slh_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', slh_analise))))
df <- df %>% mutate(se_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', se_motivo))))
df <- df %>% mutate(se_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', se_analise))))
df <- df %>% mutate(os_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', os_motivo))))
df <- df %>% mutate(os_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', os_analise))))
df <- df %>% mutate(snc_motivo = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', snc_motivo))))
df <- df %>% mutate(snc_analise = trimws(gsub(regexEspacosExtras, ' ', gsub(regexPontuacao, ' ', snc_analise))))

# Alterando valores "vazios"para "indeterminado"
df$diagnostico_presuntivo_lesao_principal_orgao <- ifelse(df$diagnostico_presuntivo_lesao_principal_orgao == '', 'indeterminado', df$diagnostico_presuntivo_lesao_principal_orgao)
df$diagnostico_presuntivo_lesao_principal_causa <- ifelse(df$diagnostico_presuntivo_lesao_principal_causa == '', 'indeterminado', df$diagnostico_presuntivo_lesao_principal_causa)
